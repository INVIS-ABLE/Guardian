...
def _stable(report: dict[str, Any]) -> dict[str, Any]:
    """Return a stable projection of ``report`` for drift comparison.

    Excludes volatile, externally-controlled data so ordinary multi-worker activity
    (another PR adding a test file, an edit shifting a line number) does not falsely
    flag the committed report as stale. The enforced invariants are the registries,
    the component summary, and the *set* of files (not line numbers) that shell out or
    carry placeholder markers — the things that actually represent the security surface.
    """
    summary = {k: v for k, v in report.get("summary", {}).items() if k != "tests"}
    return {
        ...
        "router_capabilities": report.get("router_capabilities"),
        "tool_manifest_capabilities": report.get("tool_manifest_capabilities"),
        "subprocess_files": sorted(r["file"] for r in report.get("subprocess_inventory", [])),
        "placeholder_files": sorted(r["file"] for r in report.get("placeholder_inventory", [])),
